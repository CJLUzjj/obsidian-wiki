> 原 Skill 位置：`.skills/skill-creator/references/schemas.md`
...
- `skill_name`: 正在改进的技能的名称
- `current_best`: 最佳执行者的版本标识符
...
来自评分代理的输出。位于 `<run-dir>/grading.json`。
...
- `eval_feedback`: （可选）对评估的改进建议，仅当评分器识别出值得提出的问题时才存在
...
**如何捕获：** 当子代理任务完成时，任务通知包含 `total_tokens` 和 `duration_ms`。立即保存这些信息 — 它们不会在其他任何地方持久化，事后无法恢复。
```json
{
  "total_tokens": 84852,
  "duration_ms": 23332,
...
- `runs[]`：单个运行结果
  - `eval_id`：数字评估标识符
  - `eval_name`：人类可读的评估名称（在查看器中用作部分标题）
...
- `run_summary`：每个配置的统计汇总
  - `with_skill` / `without_skill`：每个包含 `pass_rate`、`time_seconds`、`tokens` 对象，具有 `mean` 和 `stddev` 字段
  - `delta`：差异字符串，如 `"+0.50"`、`"+13.0"`、`"+1700"`
- `notes`：来自分析器的自由格式观察

**重要提示：** 查看器精确读取这些字段名称。使用 `config` 代替 `configuration`，或将 `pass_rate` 放在运行的顶级而不是嵌套在 `result` 下，将导致查看器显示空值/零值。手动生成 benchmark.json 时，始终参考此架构。

---

## comparison.json

来自盲目比较器的输出。位于 `<grading-dir>/comparison-N.json`。
...
```json
{
  "comparison_summary": {
    ...
    "comparator_reasoning": "比较者选择获胜者的原因简述"
  },
  "winner_strengths": [
    "处理多页文档的清晰分步说明",
    "包含捕获格式错误的验证脚本"
  ...
  "transcript_insights": {
    ...
    "loser_execution_pattern": "阅读技能 -> 对方法不清楚 -> 尝试了3种不同的方法"
  }
}
```
